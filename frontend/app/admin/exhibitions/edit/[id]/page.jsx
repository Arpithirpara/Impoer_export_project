...
export default function EditExhibitionPage({ params }) {
  ...
  const exhId = resolvedParams?.id || "ex1";

  const [formData, setFormData] = useState({
    title: "Gulfood 2026 Dubai International Agro Summit",
    country: "UAE",
    date: "Feb 17 - Feb 21, 2026",
  ...
  const generatedQrUrl = `https://api.qrserver.com/v1/create-qr-code/?size=240x240&data=${encodeURIComponent(
    `http://localhost:3001/exhibition/${exhId}`
  )}`;

  return (
    <AdminFormLayout
      title={`Edit Exhibition & Stall QR Code (${exhId})`}
      subtitle="Modify trade fair details, venue location, stall number, and manage live visitor QR code registration."
          ...
          <div>
            ...
            <div style={{ display: "flex", flexWrap: "wrap", gap: 10 }}>
              <a
                href={generatedQrUrl}
                target="_blank"
                download={`Stall_QR_Code_${exhId}.png`}
